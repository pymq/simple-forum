...
class BoardTopicsViewTests(TestCase):
    def setUp(self):
    ...
    def test_board_topics_url_resolves_board_topics_view(self):
        ...
        self.assertEquals(view.view_name, 'forum:board_topics')

    def test_board_topics_view_contains_navigation_links(self):
        board_topics_url = reverse('forum:board_topics', kwargs={'pk': 1})
        homepage_url = reverse('forum:index')
        new_topic_url = reverse('forum:new_topic', kwargs={'pk': 1})
...
class NewTopicTests(TestCase):
    ...
    def test_new_topic_view_success_status_code(self):
        url = reverse('forum:new_topic', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_new_topic_view_not_found_status_code(self):
        url = reverse('forum:new_topic', kwargs={'pk': 99})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 404)

    def test_new_topic_url_resolves_new_topic_view(self):
        view = resolve('/boards/1/new/')
        self.assertEquals(view.view_name, 'forum:new_topic')
